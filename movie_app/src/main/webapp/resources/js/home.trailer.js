...
async function selectTrailLinkFromServer(movieIdVal) {
    try {
        const resp = await fetch('/movie/getlink/' + movieIdVal);
...
function getTrailLink() {
    selectTrailLinkFromServer(movieIdVal).then(result => {
        document.getElementById("chatZone").innerText='';
        getChatList();
        document.getElementById("movieZone").innerHTML = `<iframe id="player" width="960" height="540" src="https://www.youtube.com/embed/${result}?enablejsapi=1" frameborder="0"></iframe>`;
...
document.addEventListener('click', (e) => {
    if (e.target.classList.contains("playBtn")) {
        movieIdVal = e.target.dataset.movieid;
        getTrailLink();
...
document.getElementById("chatSbmBtn").addEventListener('click', (e) => {
    if (chatContentObj.value == null || chatContentObj.value == '') {
        ...
        let chatData = {
            writer: document.getElementById('chatId').value,
